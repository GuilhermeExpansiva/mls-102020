...
/** Parse the page's used molecules from its `moleculeAssignments` export (unique by project|tag). */
export function parseUsedMolecules(content: string): UsedMolecule[] {
    ...
    if (!m) return [];
    let organisms: Array<{ molecules?: Array<{ project?: number; tag?: string }> }>;
    try { organisms = JSON.parse(m[1]); } catch { return []; }
    const out = new Map<string, UsedMolecule>();
    for (const org of organisms ?? []) {
        for (const mol of org?.molecules ?? []) {
            if (!mol?.tag) continue;
